...
// Requests to this downstream just delay a set number of milliseconds, defaulting to 30
async fn delayed_response(req: Request<Body>) -> Result<Response<Body>, Infallible> {
    ...
    Ok(Response::new(format!("Hello, World - *yawn* - slept for {}ms\n", millis).into()))
}
...
async fn main() {
    ...
    let make_svc = make_service_fn(|_conn| async {
        Ok::<_, Infallible>(service_fn(delayed_response))
    });
